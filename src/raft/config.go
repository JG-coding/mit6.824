...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			...
			DPrintf("old command is %v, but v command is %v", old, v)
		}
	}
...
func (cfg *config) applier(i int, applyCh chan ApplyMsg) {
	for m := range applyCh {
		if m.CommandValid == false {
			...
			cfg.mu.Lock()
			DPrintf("客户接收到 RaftNode[%d] apply的命令：%v", i, m.Command)
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		ms := 450 + (rand.Int63() % 100) //睡眠[450ms, 550ms]
		time.Sleep(time.Duration(ms) * time.Millisecond)

		leaders := make(map[int][]int) //key : term   value : []int
		for i := 0; i < cfg.n; i++ {
			if cfg.connected[i] { //该server在线
