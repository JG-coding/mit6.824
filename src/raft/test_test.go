...
func TestManyElections2A(t *testing.T) {
	...
	cfg.checkOneLeader() //检查有没有选出有且仅有一个leader

	iters := 10
	for ii := 1; ii < iters; ii++ {
		// disconnect three nodes  随机选出3个服务器脱离出集群
		i1 := rand.Int() % servers
		i2 := rand.Int() % servers
		i3 := rand.Int() % servers
		cfg.disconnect(i1)
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		DPrintf("RaftNode[%d] 掉线了", i1)
		DPrintf("RaftNode[%d] 掉线了", i2)
		DPrintf("RaftNode[%d] 掉线了", i3)
		...
		cfg.connect(i2)
		cfg.connect(i3)
		DPrintf("RaftNode[%d] 重新上线了", i1)
		DPrintf("RaftNode[%d] 重新上线了", i2)
		DPrintf("RaftNode[%d] 重新上线了", i3)
	}
...
func TestBasicAgree2B(t *testing.T) {
	...
	iters := 3
	for index := 1; index < iters+1; index++ { //1-3
		nd, _ := cfg.nCommitted(index) //验证在开始测试之前没有任何命令被错误地提交
		if nd > 0 {
...
func TestFollowerFailure2B(t *testing.T) {
	...
	cfg.one(101, servers, false) //cmd:101
	...
	DPrintf("开始测试我的失败测试:cfg.one(102, servers-1, false)")
...
func TestRejoin2B(t *testing.T) {
	...
	DPrintf("客户端追加命令: 101开始")
	cfg.one(101, servers, true)

	// leader network failure
	leader1 := cfg.checkOneLeader() //将leader退出
	DPrintf("leader: %d disconnect", leader1)
	cfg.disconnect(leader1)

	// make old leader try to agree on some entries
	DPrintf("leader: %d disconnect 后，向该leader追加102", leader1)
	cfg.rafts[leader1].Start(102)
	DPrintf("leader: %d disconnect 后，向该leader追加103", leader1)
	cfg.rafts[leader1].Start(103)
	DPrintf("leader: %d disconnect 后，向该leader追加104", leader1)
	cfg.rafts[leader1].Start(104)

	// new leader commits, also for index=2
	DPrintf("此时应该有新的leader被选出,追加命令 : 103")
	cfg.one(103, 2, true)

	// new leader network failure
	leader2 := cfg.checkOneLeader()
	DPrintf("第二个leader: %d 离线", leader2)
	cfg.disconnect(leader2)

	// old leader connected again
	cfg.connect(leader1)
	DPrintf("第一个leader: %d 重新上线", leader1)
	DPrintf("第一个leader: %d 重新上线后，客户端提交command : 104", leader1)
	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)

	DPrintf("第二个leader: %d 重新上线", leader2)
	DPrintf("第二个leader: %d 重新上线后，客户端提交command : 105", leader2)
...
func TestPersist12C(t *testing.T) {
	...
	DPrintf("客户端追加command: 11")
	cfg.one(11, servers, true) //客户端追加11
...
func TestPersist22C(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		DPrintf("客户端提交command: %d 给服务器", 10+index)
		cfg.one(10+index, servers, true)
		index++

		leader1 := cfg.checkOneLeader()
		DPrintf("leader1 := cfg.checkOneLeader()得到当前的集群的leader: %d", leader1)

		cfg.disconnect((leader1 + 1) % servers)
		cfg.disconnect((leader1 + 2) % servers)
		DPrintf("服务器：%d 断开连接", (leader1+1)%servers)
		DPrintf("服务器：%d 断开连接", (leader1+2)%servers)
		...
		cfg.disconnect((leader1 + 3) % servers)
		cfg.disconnect((leader1 + 4) % servers)
		DPrintf("服务器：%d 断开连接", (leader1+0)%servers)
		DPrintf("服务器：%d 断开连接", (leader1+3)%servers)
		DPrintf("服务器：%d 断开连接", (leader1+4)%servers)

		DPrintf("cfg.start1((leader1+1)  servers, cfg.applier)")
		cfg.start1((leader1+1)%servers, cfg.applier)
		DPrintf("cfg.start1((leader1+2)  servers, cfg.applier)")
		cfg.start1((leader1+2)%servers, cfg.applier)
		DPrintf("服务器：%d 重新连接", (leader1+1)%servers)
		DPrintf("服务器：%d 重新连接", (leader1+2)%servers)
		cfg.connect((leader1 + 1) % servers)
		cfg.connect((leader1 + 2) % servers)

		time.Sleep(RaftElectionTimeout)

		DPrintf("cfg.start1((leader1+3)  servers, cfg.applier)")
		cfg.start1((leader1+3)%servers, cfg.applier)
		DPrintf("服务器：%d 重新连接", (leader1+3)%servers)
		cfg.connect((leader1 + 3) % servers)
		...
		DPrintf("服务器：%d 重新连接", (leader1+4)%servers)
		DPrintf("服务器：%d 重新连接", (leader1+0)%servers)
		cfg.connect((leader1 + 4) % servers)
		cfg.connect((leader1 + 0) % servers)
	}

	DPrintf("执行最后一个命令，客户端提交command: %d 给服务器", 1000)
	cfg.one(1000, servers, true)
...
func TestFigure82C(t *testing.T) {
	...
	DPrintf("cfg.one(rand.Int(), 1, true)")
	cfg.one(rand.Int(), 1, true)

	nup := servers
	for iters := 0; iters < 1000; iters++ {
		leader := -1
		for i := 0; i < servers; i++ { //只能提交到给leader
			if cfg.rafts[i] != nil {
		...
		if (rand.Int() % 1000) < 100 { //10%的概率执行这个
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	DPrintf("-------------cfg.one(rand.Int() 10000, 1, true)---------")
	nup := servers
	for iters := 0; iters < 1000; iters++ {
		if iters == 200 {
		...
		for i := 0; i < servers; i++ {
			...
			if ok && cfg.connected[i] {
				DPrintf("------------------------------------------------------")
				DPrintf("---------if ok && cfg.connected[i] ,leader为 %d--------------", i)
				leader = i
			}
		}
		...
		if nup < 3 {
			DPrintf("------------------------------------------------------")
			DPrintf("-----------------集群中可用服务器少于3个---------------------")
			s := rand.Int() % servers
			if cfg.connected[s] == false {
				cfg.connect(s)
				DPrintf("-----------------集群中可用服务器少于3个，上线node %d--------------", s)
				nup += 1
			}
		}
	}
